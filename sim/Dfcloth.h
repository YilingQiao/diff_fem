...
public:
	...
	void SetDfobj(const std::string& path);
...
SetDfobj(const std::string& path)
{
    ...
    mParameter.area_density = TinyConstants::scalar_from_doble(
        jsonRequireDouble(object, "Area Density"));
    mParameter.bend = TinyConstants::scalar_from_doble(
        jsonRequireDouble(object, "Bending"));
    mParameter.stretch = TinyConstants::scalar_from_doble(
    ...
    T(0,0) *= TinyConstants::scalar_from_doble(0.05); 
    T(1,1) *= TinyConstants::scalar_from_doble(0.05);
    T(2,2) *= TinyConstants::scalar_from_doble(0.05); 
...
SetMesh(const std::string& path,const Eigen::Transform<TinyScalar, 3, 2>& T)
{
    ...
    for (int i = 0; i < mMesh->mVertices.size(); i++) {
        positions.push_back(mMesh->mVertices[i]);
    }
    for (int i = 0; i < mMesh->mTriangles.size(); i++) {
        triangles.push_back(mMesh->mTriangles[i][0]);
        triangles.push_back(mMesh->mTriangles[i][1]);
        triangles.push_back(mMesh->mTriangles[i][2]);
    }
