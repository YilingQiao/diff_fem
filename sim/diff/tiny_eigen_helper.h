...
template <typename TinyScalar, typename TinyConstants>
static std::vector<Eigen::Matrix<double, 3, 1> > 
to_eigen_double(
    const std::vector<Eigen::Matrix<TinyScalar, 3, 1>>& m) {
  std::vector<Eigen::Matrix<double, 3, 1>> result(m.size());
  for (int v = 0; v < result.size(); v++) {
    result[v] = Eigen::Matrix<double, 3, 1>();
    for (int i = 0; i < m[v].rows(); ++i) {
      for (int j = 0; j < m[v].cols(); ++j) {
        result[v](i, j) = TinyConstants::getDouble(m[v](i, j));
      }
    }
  }
  return result;
...
template <typename TinyScalar, typename TinyConstants>
static  std::vector<Eigen::Matrix<TinyScalar, Eigen::Dynamic, 1> >
to_eigen_double2tiny(
    const std::vector<Eigen::Matrix<double, Eigen::Dynamic, 1> >& m) {
...
template <typename TinyScalar, typename TinyConstants>
static Eigen::Matrix<double, Eigen::Dynamic, 1> to_eigen_double(
    const Eigen::Matrix<TinyScalar, Eigen::Dynamic, 1>& m) {
  Eigen::Matrix<double, Eigen::Dynamic, 1> result(m.rows(), m.cols());
  for (int i = 0; i < m.rows(); ++i) {
    for (int j = 0; j < m.cols(); ++j) {
      result(i, j) = TinyConstants::getDouble(m(i, j));
    }
  }
  return result;
}
