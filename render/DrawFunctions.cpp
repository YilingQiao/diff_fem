...
void 
GUI::
DrawCollisionMesh(World<double, DoubleUtils>* world,const Eigen::Vector3d& eye)
{
	glEnable(GL_LIGHTING); 

	CollisionMesh<double, DoubleUtils>* 	mCollisionMesh = world->mCollisionMesh;

	Eigen::VectorXd& x = mCollisionMesh->m_positions;
	const std::vector<Eigen::Vector3i>& contours = mCollisionMesh->mContours;

	std::vector<std::pair<int,double>> sorted_contours;
	const Eigen::VectorXd& v_n = mCollisionMesh->mVertexNormal;
...
DrawCharacter(Dfobj<double, DoubleUtils>* Dfobj,const Eigen::VectorXd& x,const Eigen::Vector3d& eye)
{
	...
	const std::vector<Eigen::Vector3i>& contours = Dfobj->GetContours();
	std::vector<std::pair<int,double>> sorted_contours;
	const Eigen::VectorXd& v_n = Dfobj->GetVertexNormal();
...
void 
GUI::
DrawMuscles(const std::vector<Muscle<double, DoubleUtils>*>& muscles,const Eigen::VectorXd& x)
{
	glEnable(GL_LIGHTING);
...
void 
GUI::
DrawActivations(const double& x, const double& y,const double& length,
	const double& height,Muscle<double, DoubleUtils>* muscle1,Muscle<double, DoubleUtils>* muscle2)
{
	glDisable(GL_LIGHTING);
