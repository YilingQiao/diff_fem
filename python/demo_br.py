...
parser.add_argument('--inp', default='bridge', help='input json')
parser.add_argument('--out', default='bridge_demo', help='output dir')
...
def main():
    json_path = "/data/bridge/{}.json".format(args.inp)
    ...
    n_cora = 2911
    init_tau = torch.tensor([1,1]*n_cora, dtype=torch.float32, requires_grad=True)
    t_scale = torch.tensor([2e5,0.4]*n_cora, dtype=torch.float32)
